...
def generate_human36m_adjacency_matrix(num_joints=17):
    # 定义Human3.6M中各关节的连接关系
    adjacency = np.zeros((num_joints, num_joints))

    # 基于人体关节的拓扑结构创建邻接矩阵
    # 连接关系（示例，具体连接根据实际数据集关节顺序）
    connections = [
        (0, 1), (1, 2), (2, 3), (3, 4),  # 右手
        (0, 5), (5, 6), (6, 7), (7, 8),  # 左手
        (1, 5), (5, 9), (9, 10),  # 躯干
    ...
    for connection in connections:
        adjacency[connection[0], connection[1]] = 1
        adjacency[connection[1], connection[0]] = 1  # 对称关系
...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    # 生成邻接矩阵
    adjacency = generate_human36m_adjacency_matrix(num_joints=opt.out_joints).cuda()
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if split == 'train':
            # 将邻接矩阵传入模型
            output_3D, output_3D_VTE = model_trans(input_2D, adjacency)
        else:
